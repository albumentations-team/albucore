...
from albucore.decorators import (
    restore_from_spatial, restore_from_channel, reshape_for_spatial, reshape_for_channel
)
...
SPATIAL_SHAPES = [
    ...
    ((10, 5, 32, 32, 1), (32, 32, 50), True, True),  # N,D,H,W,C (grayscale)
]
...
CHANNEL_SHAPES = [
    ...
    ((10, 5, 32, 32, 1), (1600, 32, 1), True, True),  # N,D,H,W,C (grayscale)
]
...
def test_channel_transform_preserves_width():
    ...
    restored = restore_from_channel(transformed, original_shape)
...
SPATIAL_3D_SHAPES = [
    ...
    # Note: keep_depth_dim only works with 5D arrays (NDHWC) now
    ((10, 5, 32, 32, 3), (5, 32, 32, 30), True, True, True),  # N,D,H,W,C => D,H,W,N*C
    ((10, 5, 32, 32, 1), (5, 32, 32, 10), True, True, True),  # N,D,H,W,C => D,H,W,N*C (grayscale)
]
...
def test_spatial_3d_roundtrip(
    ...
    restored = restore_from_spatial(
        reshaped, original_shape, keep_depth_dim=keep_depth
    )
...
def test_spatial_3d_transform_preserves_dims():
    ...
    # Simulate a spatial transform that changes H,W dimensions
    reshaped, original_shape = reshape_for_spatial(
        data, keep_depth_dim=True
    )
    # Double H,W dimensions
    transformed = np.random.rand(5, 64, 64, 30)  # (D,H',W',N*C)

    restored = restore_from_spatial(
        transformed, original_shape, keep_depth_dim=True
    )
...
def test_spatial_3d_transform_preserves_order():
    ...
    restored = restore_from_spatial(
        reshaped, original_shape, keep_depth_dim=True
    )
