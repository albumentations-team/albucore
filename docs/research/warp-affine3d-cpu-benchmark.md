...
The Tensor run compared two diagnostic alternatives. Manual broadcasted grid construction was faster in 1 of 64 cells and had a uint8 rounding difference at a sampling boundary. It fails both the stable-win and exact-parity gates. The coverage sampler won 3 of 16 nonzero-fill cells. On a contiguous `5×16×128×160` float32 Tensor with nonzero fill, native sampling took 3.839 ms and the coverage sampler took 5.417 ms. Production keeps the shifted-input fill adapter, which uses one data sampler instead of data plus coverage sampling.
...
| Coverage sampler for nonzero fill | Rejected | A second grid-sample call loses on 13 of 16 nonzero-fill quick cells, including the large representative shape. |
