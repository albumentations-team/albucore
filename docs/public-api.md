...
- **Geometric:** `copy_make_border`, `gaussian_blur3d`, `remap`, `resize`, `resize3d`, `separable_filter3d`, `warp_affine`, `warp_affine3d`, `warp_perspective`. `gaussian_blur3d` and `separable_filter3d` filter one prevalidated NumPy `DHWC` or CPU Torch `CDHW` volume through three float32 D/H/W passes with fixed `BORDER_REFLECT_101` semantics, then restore uint8 once. They preserve their supported uint8 and float32 dtypes. An unexpected float64 volume is converted to and returned as float32. `gaussian_blur3d` accepts scalar or `(D,H,W)` sigma and kernel-size controls; zero sigma skips that axis. Neither function accepts batched `NDHWC`/`NCDHW` layouts or implements target-level mask behavior. `resize3d` expects prevalidated NumPy `DHWC` or Torch `CDHW`, supports uint8/float32, and runs Tensor kernels in inference mode. Large linear all-axis Tensor upscales (at least 10,000 output elements) use a zero-copy NumPy/OpenCV bridge selected by benchmark; float32 remains within `rtol=2e-4`, `atol=3e-5` of native Torch and uint8 differs by at most one value. `warp_affine3d` applies one forward voxel-space `(x, y, z)` matrix to one prevalidated NumPy `DHWC` or CPU Torch `CDHW` volume. The upstream caller supplies uint8/float32 data, a valid matrix, size, interpolation, border mode, and fill; masks use the same dtype rule, so AlbumentationsX rejects int64 before this call. `warp_affine3d` never accepts batched `NDHWC` or `NCDHW` layouts. It does not repeat CPU, layout, or autograd validation and never moves or detaches Tensor data. All four routers preserve container, layout, and channels; their dtype preservation applies to their documented supported dtypes.
