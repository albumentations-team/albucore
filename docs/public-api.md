...
- **3D dense resampling:** `remap3d` applies a caller-provided float32 `(D_out, H_out, W_out, 3)` normalized `align_corners=False` pull grid in `(x, y, z)` order to one prevalidated NumPy `DHWC` volume. The grid can be NumPy or Torch, its spatial shape defines the output, and coordinates outside `[-1, 1]` use constant or replicate borders. The router preserves dtype, channel count, and `DHWC` layout. It does not scan the dense grid for identity or convert a voxel-coordinate map. The CPU Torch volume overload was benchmarked but is intentionally not public because it lost to the zero-copy NumPy bridge on the required route gate.
