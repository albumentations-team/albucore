...
| [`benchmark_numkong_astype.py`](benchmark_numkong_astype.py) | NumKong 7.8 buffer-first cast: previous NumPy `rint`+clip and isolated cast paths across HWC/XHWC contiguous and strided inputs. |
...
| [`benchmark_remap3d.py`](benchmark_remap3d.py) | Complete CPU NumPy `DHWC` `remap3d` routes with NumPy/Tensor grids, raw timings, and RSS; the rejected Tensor-volume gate is recorded separately. |
...
For `remap3d`, start with the development matrix, then use `--full` for the accepted NumPy-route matrix. The rejected CPU Tensor-volume candidate is documented in [`benchmark_remap3d-route-decision.md`](results/benchmark_remap3d-route-decision.md):

```bash
uv run python benchmarks/benchmark_remap3d.py --quick --threads 1
uv run python benchmarks/benchmark_remap3d.py --full --threads 1 --output benchmarks/results/benchmark_remap3d.md
```
